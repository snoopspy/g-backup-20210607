...
// GPropItemObjPtrList
// ----------------------------------------------------------------------------
GPropItemObjPtrList::GPropItemObjPtrList(QTreeWidgetItem* parent, QObject* object, QMetaProperty mpro)
  : GPropItemToolButton(parent, object, mpro) {
  toolButton_->setText("+");
  QObject::connect(toolButton_, SIGNAL(clicked(bool)), this, SLOT(myClicked(bool)));
}

void GPropItemObjPtrList::update() {
  ...
  for (_GObjPtrList::iterator it = list->begin(); it != list->end(); it++) {
    ...
    childObj->createPropItems(mgr);
  }
}

void GPropItemObjPtrList::myClicked(bool checked) {
  (void)checked;
  const char* propName = mpro_.name();
  QVariant variant = object_->property(propName);
  GObjPtrListPtr list = qvariant_cast<GObjPtrListPtr>(variant);
